...
def fetch_commits_from_local_repo(repo_path):
    commit_list = []
    result = subprocess.Popen(['git', '-C', repo_path, 'log', '--format=%B'], stdout=subprocess.PIPE)
    for i, commit_message in enumerate(result.stdout):
        commit_message = commit_message.decode('utf-8').strip()
        commit_list.append({'id': i + 1, 'message': commit_message})  # Assign unique ID starting from 1
    return commit_list
